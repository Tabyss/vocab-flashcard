...
export default function App() {
  ...
  const isWordExists = useMemo(() => {
    return allCards.some(
      (c) => c.word?.toLowerCase() === searchTerm.trim().toLowerCase(),
    );
  }, [searchTerm, allCards]);
        ...
        <div className="sticky top-0 w-full z-20 flex justify-center px-6 py-4 bg-gray-50/80 dark:bg-gray-950/80 backdrop-blur-md dark:border-gray-800 shadow-sm">
          <div className="w-full max-w-2xl relative">
              ...
              <AnimatePresence>
                {searchTerm.trim() && !isWordExists && (
                  <motion.button
                    ...
                    className="absolute right-2 top-2 bottom-2 px-5 bg-blue-600 text-white text-sm font-bold rounded-xl hover:bg-blue-500 transition-all shadow-lg shadow-blue-500/20"
                  >
                    + Add New
                  </motion.button>
                )}
            ...
            {searchTerm && (
              ...
              >
                {isWordExists ? (
                  <span className="text-[10px] text-green-500 font-bold uppercase tracking-wider bg-green-50 dark:bg-green-900/20 px-2 py-0.5 rounded">
                    ✓ In Library
                  </span>
                ) : (
                  <span className="text-[10px] text-blue-500 font-bold uppercase tracking-wider bg-blue-50 dark:bg-blue-900/20 px-2 py-0.5 rounded">
                    ✨ New Word
                  </span>
                )}
              </motion.div>
        ...
        <div className="w-full max-w-6xl px-6 pt-12 pb-20">
          ...
          {filteredCards.length === 0 && (
            <div className="text-center py-24 opacity-20">
              ...
              <p className="text-xl font-bold">No vocabulary found.</p>
            </div>
          )}
        </div>
      </main>
